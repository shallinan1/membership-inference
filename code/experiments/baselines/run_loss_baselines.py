        ...
        if strategy == "ReferenceLoss":
            if args.ref_model_probs is not None:
                for ref_model_path in args.ref_model_probs:
                    ...
                    for target_fpr in [0.005, 0.01, 0.05]:  # 0.5%, 1%, 5%
                        ...
                        all_scores[strategy][ref_model_name][f"tpr_at_{int(target_fpr*100)}pct_fpr"] = tpr_at_fpr
            ...
            for target_fpr in [0.0001, 0.005, 0.01, 0.05]:  # 0.5%, 1%, 5%
                ...
                if len(valid_indices) > 0:
                    ...
                    tpr_at_fpr = 0.0  # If no FPR <= target, set TPR to 0
                all_scores[strategy][f"tpr_at_{int(target_fpr*1000)}pct_fpr"] = tpr_at_fpr
